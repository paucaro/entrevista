...
class MiningText {
  ...
  MiningText(String text) {
    final String temp = caseFolding(text);
    final List<String> filtered = filtering(temp);
  ...
  List<String> filtering(String text) {
    ...
    final List<String> significantWords = text.replaceAll('\\b(' + pattern + ')\\b', '').split(' ');
    final SynonimWord synonimWord = SynonimWord();
    final BTree treeSynonims = synonimWord.returnBTreeSynonim();
